export class GameboyCross extends HTMLElement {
    constructor() {
      super();
    ...
    addEventListeners() {
      const axisX = this.shadowRoot.querySelector('.axis.x');
      const axisY = this.shadowRoot.querySelector('.axis.y');
  
      axisX.addEventListener('click', (e) => {
        const rect = axisX.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const direction = x < rect.width / 2 ? 'LEFT' : 'RIGHT';
        window.dispatchEvent(new CustomEvent('gameboyButtonPress', {
          detail: { button: direction }
        }));
      });
  
      axisY.addEventListener('click', (e) => {
        const rect = axisY.getBoundingClientRect();
        const y = e.clientY - rect.top;
        const direction = y < rect.height / 2 ? 'UP' : 'DOWN';
        window.dispatchEvent(new CustomEvent('gameboyButtonPress', {
          detail: { button: direction }
        }));
      });
